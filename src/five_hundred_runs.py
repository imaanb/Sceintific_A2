...
def calculate_fractal_dim_for_eta(eta):
    ...
    for i in range(iters):
        ...
        growth_evolution = dla(c, cluster_grid, 1.75, N, steps, eta)
        final_frame = growth_evolution[steps-1]
    ...
    print(f"frac dim for eta {eta}: {mean}")
    return eta, mean
...
def main():
    ...
    for eta, mean_frac_dim in results:
        fractal_dims[eta] = mean_frac_dim

    # Save the results to a pickle file
    with open('../runs/final_no_early_stop.pkl', 'wb') as f:
